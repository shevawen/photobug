...
public class UserModule extends BaseModule {
	...
	public Object login_action(@Param("name") String name, @Param("password") String password, HttpSession session) {
		NutMap re = new NutMap();
		User user = dao.fetch(User.class, Cnd.where("name", "=", name).and("password", "=", Helper.MD5(password)));
		if (user == null) {
			re.setv("ok", false);
		} else {
	...
	@At
	public Object delete(@Param("name") int name, @Attr("me") String me) {
		if (me.equals(name)) {
			return new NutMap().setv("ok", false).setv("msg", "不能删除当前用户!!");
		}
		dao.delete(User.class, name); // 再严谨一些的话,需要判断是否为>0
